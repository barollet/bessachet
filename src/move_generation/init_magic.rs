...
pub fn fill_attack_table(attack_table: &mut SlidingAttackTable, square: u8) {
    let magic: u64 = ROOK_MAGIC[usize::from(square)].0;
    let table_pointer: *mut BitBoard = &mut attack_table[ROOK_MAGIC[usize::from(square)].1];
    fill_attack_table_helper(
        square,
        table_pointer,
        magic,
        rook_mask,
    ...
    let table_pointer: *mut BitBoard = &mut attack_table[BISHOP_MAGIC[usize::from(square)].1];
    fill_attack_table_helper(
        square,
        table_pointer,
        magic,
...
fn fill_attack_table_helper(
    square: u8,
    table_pointer: *mut BitBoard,
    magic: u64,
    get_mask: fn(u8) -> BitBoard,
    get_offset: fn(BitBoard, u64) -> usize,
    get_attack: fn(u8, BitBoard) -> BitBoard,
) {
    ...
    for i in 0..(1 << n) {
        ...
        unsafe {
            let old_value: BitBoard = ptr::read(table_pointer.add(table_index));
...
fn direction_blockers_mask<F, G>(
    result: &mut BitBoard,
    blockers: BitBoard,
    mut kl: (u8, u8),
    update: F,
