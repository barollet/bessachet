...
fn perft_bench() {
    ...
    assert_eq!(board.perft(6), 8_031_647_685);

    // Sparse board
    let mut board = Board::from_fen("8/2p5/3p4/KP5r/1R3p1k/8/4P1P1/8 w - - ").unwrap();

    assert_eq!(board.perft(8), 3_009_794_393);
    ...
    assert_eq!(board.perft(6), 706_045_033);
    ...
    assert_eq!(board.perft(6), 6_923_051_137);
}
...
fn check_pin_test() {
    ...
    assert_eq!(board.move_gen.number_of_pinned_pieces, 0);
    assert_eq!(board.move_gen.number_of_checkers, 0);
    ...
    assert_eq!(board.move_gen.number_of_pinned_pieces, 1);
    assert_eq!(board.move_gen.number_of_checkers, 0);
    ...
    let board = Board::from_fen("r3k2r/Pppp1ppp/1b3nbN/nP6/BBP1P3/q4N2/Pp1P2PP/R2Q1RK1 w kq - 0 1")
        .unwrap();
    assert_eq!(board.move_gen.number_of_pinned_pieces, 0);
    assert_eq!(board.move_gen.number_of_checkers, 1);
    ...
    assert_eq!(board.move_gen.number_of_pinned_pieces, 5);
    assert_eq!(board.move_gen.number_of_checkers, 0);
}
